submodule (MOM_remapping) MOM_remapping_s
  implicit none
contains
module procedure remapping_set_param
...
end procedure remapping_set_param
module procedure extract_member_remapping_CS
...
end procedure extract_member_remapping_CS
module procedure remapping_core_h
...
end procedure remapping_core_h
module procedure remapping_core_w
  real, dimension(n0+n1+1) :: h_sub ! Width of each each sub-cell [H]
  real, dimension(n0+n1+1) :: uh_sub ! Integral of u*h over each sub-cell [A H]
  real, dimension(n0+n1+1) :: u_sub ! Average of u over each sub-cell [A]
  integer, dimension(n0+n1+1) :: isub_src ! Index of source cell for each sub-cell
...
end procedure remapping_core_w
module procedure build_reconstructions_1d
...
end procedure build_reconstructions_1d
module procedure check_reconstructions_1d
  integer :: i0, n
...
end procedure check_reconstructions_1d
module procedure intersect_src_tgt_grids
...
end procedure intersect_src_tgt_grids
module procedure remap_src_to_sub_grid_om4
  integer :: i_sub ! Index of sub-cell
  integer :: i0 ! Index into h0(1:n0), source column
  integer :: i_max ! Used to record which sub-cell is the largest contribution of a source cell
  real :: dh_max ! Used to record which sub-cell is the largest contribution of a source cell [H]
...
end procedure remap_src_to_sub_grid_om4
module procedure remap_src_to_sub_grid
  integer :: i_sub ! Index of sub-cell
  integer :: i0 ! Index into h0(1:n0), source column
  integer :: i_max ! Used to record which sub-cell is the largest contribution of a source cell
  real :: dh_max ! Used to record which sub-cell is the largest contribution of a source cell [H]
...
end procedure remap_src_to_sub_grid
module procedure remap_sub_to_tgt_grid_om4
...
end procedure remap_sub_to_tgt_grid_om4
module procedure remap_sub_to_tgt_grid
  integer :: i1 ! tgt loop index
  integer :: i_sub ! index to sub-layer
  real :: dh ! The width of the sub-cell [H]
  real :: duh ! The total amount of accumulated stuff (u*h)  [A H]
...
end procedure remap_sub_to_tgt_grid
module procedure interpolate_column
...
end procedure interpolate_column
module procedure reintegrate_column
...
end procedure reintegrate_column
module procedure average_value_ppoly
...
end procedure average_value_ppoly
module procedure check_remapped_values
...
end procedure check_remapped_values
module procedure measure_input_bounds
...
end procedure measure_input_bounds
module procedure measure_output_bounds
  real :: eps  ! The smallest representable fraction of a number [nondim]
  integer :: k
  eps = epsilon(h1(1))
  h1tot = h1(1)
...
end procedure measure_output_bounds
module procedure dzFromH1H2
  integer :: k
...
end procedure dzFromH1H2
module procedure initialize_remapping
...
end procedure initialize_remapping
module procedure setReconstructionType
  integer :: degree
  degree = -99
...
end procedure setReconstructionType
module procedure end_remapping
  CS%degree = 0

end procedure end_remapping
module procedure test_interp
...
end procedure test_interp
module procedure test_reintegrate
...
end procedure test_reintegrate
module procedure test_recon_consistency
...
end procedure test_recon_consistency
module procedure test_preserve_uniform
  type(remapping_CS) :: remapCS !< Remapping control structure
...
end procedure test_preserve_uniform
module procedure test_unchanged_grid
  type(remapping_CS) :: remapCS !< Remapping control structure
  real :: h0(n0), h1(n0) ! Source and target grids [H but really nondim]
  real :: u0(n0), u1(n0) ! Source and target values [A]
  logical :: error ! Indicates a divergence
...
end procedure test_unchanged_grid
module procedure compare_two_schemes
...
end procedure compare_two_schemes
module procedure remapping_unit_tests
  integer :: n0, n1, n2
...
end procedure remapping_unit_tests
end submodule MOM_remapping_s
