submodule (MOM_tracer_diabatic) MOM_tracer_diabatic_s
#include <MOM_memory.h>
  implicit none
contains
module procedure tracer_vertdiff
...
end procedure tracer_vertdiff
module procedure tracer_vertdiff_Eulerian
  real :: sink_dist !< The distance the tracer sinks in a time step [H ~> m or kg m-2].
  real, dimension(SZI_(G),SZJ_(G)) :: &
    sfc_src, &      !< The time-integrated surface source of the tracer [CU H ~> CU m or CU kg m-2].
    btm_src         !< The time-integrated bottom source of the tracer [CU H ~> CU m or CU kg m-2].
...
end procedure tracer_vertdiff_Eulerian
module procedure applyTracerBoundaryFluxesInOut
...
end procedure applyTracerBoundaryFluxesInOut
end submodule MOM_tracer_diabatic_s
