submodule (MOM_internal_tides) MOM_internal_tides_s
#include <MOM_memory.h>
  implicit none
contains
module procedure propagate_int_tide
...
end procedure propagate_int_tide
module procedure sum_En
...
end procedure sum_En
module procedure itidal_lowmode_loss
...
end procedure itidal_lowmode_loss
module procedure get_lowmode_loss
...
end procedure get_lowmode_loss
module procedure get_lowmode_diffusivity
...
end procedure get_lowmode_diffusivity
module procedure refract
  ...
  ! Update and copy back to En.
    ...
    enddo ; enddo
  enddo ! j-loop
end procedure refract
module procedure PPM_angular_advect
  ...
  enddo
end procedure PPM_angular_advect
...
end procedure propagate_x
module procedure propagate_y
  real, dimension(SZI_(G),SZJ_(G)) :: &
...
end procedure propagate_y
module procedure zonal_flux_En
  ...
  enddo
end procedure zonal_flux_En
module procedure merid_flux_En
  real :: CFL ! The CFL number based on the local velocity and grid spacing [nondim].
  real :: curv_3 ! A measure of the energy density curvature over a grid length [H Z2 T-2 ~> m3 s-2 or J m-2]
  integer :: i
  do i=ish,ieh
  ...
  enddo
end procedure merid_flux_En
module procedure reflect
...
end procedure reflect
module procedure turning_latitude
  real, dimension(G%isd:G%ied,G%jsd:G%jed) :: angle_c
  real, dimension(1:Nangle) :: En_reflected ! Energy reflected [H Z2 T-2 ~> m3 s-2 or J m-2].
  real    :: TwoPi                         ! 2*pi = 6.2831853... [nondim]
  real    :: Angle_size                    ! size of beam wedge [rad]
...
end procedure turning_latitude
module procedure teleport
  real, dimension(G%isd:G%ied,G%jsd:G%jed)    :: angle_c
  real, dimension(G%isd:G%ied,G%jsd:G%jed)    :: part_refl
...
end procedure teleport
module procedure correct_halo_rotation
  ...
  enddo
end procedure correct_halo_rotation
module procedure correct_halo_rotation_2d
  real, dimension(G%isd:G%ied,NAngle) :: En2d ! A zonal row of the internal gravity wave energy density
  integer, dimension(G%isd:G%ied) :: a_shift
  integer :: i_first, i_last, a_new
  ...
  enddo
end procedure correct_halo_rotation_2d
module procedure PPM_reconstruction_x
...
end procedure PPM_reconstruction_x
module procedure PPM_reconstruction_y
  real, dimension(SZI_(G),SZJ_(G))  :: slp ! The slope in energy density times the cell width
  real, parameter :: oneSixth = 1./6. ! One sixth [nondim]
...
end procedure PPM_reconstruction_y
module procedure PPM_limit_pos
  ...
  enddo ; enddo
end procedure PPM_limit_pos
module procedure minmod_limiter
...
end procedure minmod_limiter
module procedure register_int_tide_restarts
...
end procedure register_int_tide_restarts
module procedure internal_tides_init
...
end procedure internal_tides_init
module procedure internal_tides_end
...
end procedure internal_tides_end
end submodule MOM_internal_tides_s
