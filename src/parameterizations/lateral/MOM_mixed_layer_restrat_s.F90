submodule (MOM_mixed_layer_restrat) MOM_mixed_layer_restrat_s
#include <MOM_memory.h>
  implicit none
contains
module procedure mixedlayer_restrat
...
end procedure mixedlayer_restrat
module procedure mixedlayer_restrat_OM4
...
end procedure mixedlayer_restrat_OM4
module procedure mu
...
module procedure mixedlayer_restrat_Bodner
...
end procedure mixedlayer_restrat_Bodner
module procedure rmean2ts
...
end procedure rmean2ts
module procedure mixedlayer_restrat_BML
  real :: uhml(SZIB_(G),SZJ_(G),SZK_(GV)) ! Restratifying zonal thickness transports [H L2 T-1 ~> m3 s-1 or kg s-1]
  real :: vhml(SZI_(G),SZJB_(G),SZK_(GV)) ! Restratifying meridional thickness transports [H L2 T-1 ~> m3 s-1 or kg s-1]
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)) :: &
    h_avail               ! The volume available for diffusion out of each face of each
...
end procedure mixedlayer_restrat_BML
module procedure detect_mld
  ...
  do j=js-1,je+1
    ...
    enddo
  enddo ! j-loop
end procedure detect_mld
module procedure growth_time
  real :: mom_mixrate  ! rate at which momentum is homogenized within mixed layer [T-1 ~> s-1]
...
end procedure growth_time
module procedure mixedlayer_restrat_init
...
end procedure mixedlayer_restrat_init
module procedure mixedlayer_restrat_register_restarts
...
end procedure mixedlayer_restrat_register_restarts
module procedure mixedlayer_restrat_unit_tests
  logical :: this_test
...
end procedure mixedlayer_restrat_unit_tests
module procedure test_answer
...
end procedure test_answer
end submodule MOM_mixed_layer_restrat_s
