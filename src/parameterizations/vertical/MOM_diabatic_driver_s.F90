submodule (MOM_diabatic_driver) MOM_diabatic_driver_s
#include <MOM_memory.h>
  implicit none
contains
module procedure diabatic
...
end procedure diabatic
module procedure diabatic_ALE_legacy
...
end procedure diabatic_ALE_legacy
module procedure diabatic_ALE
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)) :: &
    h_orig, &    ! Initial layer thicknesses [H ~> m or kg m-2]
    dz,     &    ! The vertical distance between interfaces around a layer [Z ~> m]
    dSV_dT, &    ! The partial derivative of specific volume with temperature [R-1 C-1 ~> m3 kg-1 degC-1]
...
end procedure diabatic_ALE
module procedure layered_diabatic
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)) :: &
...
end procedure layered_diabatic
module procedure extract_diabatic_member
...
end procedure extract_diabatic_member
module procedure adiabatic
...
module procedure diagnose_diabatic_diff_tendency
...
end procedure diagnose_diabatic_diff_tendency
module procedure diagnose_boundary_forcing_tendency
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)) :: work_3d ! A 3-d work array for diagnostics [various]
  real, dimension(SZI_(G),SZJ_(G))          :: work_2d ! A 2-d work array for diagnostics [various]
  real :: Idt  ! The inverse of the timestep [T-1 ~> s-1]
  integer :: i, j, k, is, ie, js, je, nz
...
end procedure diagnose_boundary_forcing_tendency
module procedure diagnose_frazil_tendency
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)) :: work_3d ! A 3-d work array for diagnostics [various]
  real, dimension(SZI_(G),SZJ_(G))          :: work_2d ! A 2-d work array for diagnostics [various]
  real    :: Idt ! The inverse of the timestep [T-1 ~> s-1]
  integer :: i, j, k, is, ie, js, je, nz
...
end procedure diagnose_frazil_tendency
module procedure adiabatic_driver_init
...
end procedure adiabatic_driver_init
module procedure diabatic_driver_init
...
end procedure diabatic_driver_init
module procedure register_diabatic_restarts
...
end procedure register_diabatic_restarts
module procedure diabatic_driver_end
...
end procedure diabatic_driver_end
end submodule MOM_diabatic_driver_s
