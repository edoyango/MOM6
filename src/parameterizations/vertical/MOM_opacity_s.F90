submodule (MOM_opacity) MOM_opacity_s
#include <MOM_memory.h>
  implicit none
contains
module procedure set_opacity
...
end procedure set_opacity
module procedure opacity_from_chl
...
end procedure opacity_from_chl
module procedure opacity_morel
...
end procedure opacity_morel
module procedure SW_pen_frac_morel
  real :: Chl, Chl2         ! The log10 of chl_data in mg m-3, and Chl^2 [nondim]
  Chl = log10(min(max(chl_data,0.02),60.0)) ; Chl2 = Chl*Chl
...
end procedure SW_pen_frac_morel
module procedure extract_optics_slice
...
end procedure extract_optics_slice
module procedure extract_optics_fields
  if (present(nbands)) nbands = optics%nbands

end procedure extract_optics_fields
module procedure optics_nbands
  if (associated(optics)) then
    optics_nbands = optics%nbands
  else
    optics_nbands = 0
  endif
end procedure optics_nbands
module procedure absorbRemainingSW
...
end procedure absorbRemainingSW
module procedure sumSWoverBands
...
end procedure sumSWoverBands
module procedure opacity_init
...
end procedure opacity_init
module procedure init_ohlmann_table
  ...
  return
end procedure init_ohlmann_table
module procedure lookup_ohlmann_swpen
  real :: log10chl
  integer :: n
...
end procedure lookup_ohlmann_swpen
module procedure lookup_ohlmann_opacity
  real :: log10chl
  integer :: n
  if (chl > optics%chl_min) then
    log10chl = min(log10(chl),optics%log10chl_max)
  ...
  return
end procedure lookup_ohlmann_opacity
module procedure opacity_end
...
end procedure opacity_end
end submodule MOM_opacity_s
