submodule (MOM_energetic_PBL) MOM_energetic_PBL_s
#include <MOM_memory.h>
  implicit none
contains
module procedure energetic_PBL
...
end procedure energetic_PBL
module procedure ePBL_column
  real, dimension(SZK_(GV)+1) :: &
...
end procedure ePBL_column
module procedure ePBL_BBL_column
  real, dimension(SZK_(GV)+1) :: &
    pres_Z, &       ! Interface pressures with a rescaling factor to convert interface height
                    ! movements into changes in column potential energy [R Z2 T-2 ~> kg m-1 s-2].
...
end procedure ePBL_BBL_column
module procedure kappa_eqdisc
  ...
  enddo
end procedure kappa_eqdisc
module procedure get_eqdisc_v0
...
end procedure get_eqdisc_v0
module procedure get_eqdisc_v0h
  real :: bflux_c  ! capped bflux [Z2 T-3 ~> m2 s-3]
  ...
  ! some cause issues such as anomlous surface warming.
  ! this needs further investigation, our choices are motivated by practicallity for now.
end procedure get_eqdisc_v0h
module procedure exp_decay_TKE_adjust
...
end procedure exp_decay_TKE_adjust
module procedure find_PE_chg
...
end procedure find_PE_chg
module procedure find_Kd_from_PE_chg
...
end procedure find_Kd_from_PE_chg
module procedure find_PE_chg_orig
...
end procedure find_PE_chg_orig
module procedure find_mstar
...
end procedure find_mstar
module procedure mstar_Langmuir
...
end procedure mstar_Langmuir
module procedure energetic_PBL_get_MLD
...
end procedure energetic_PBL_get_MLD
module procedure energetic_PBL_init
...
end procedure energetic_PBL_init
module procedure energetic_PBL_end
...
end procedure energetic_PBL_end
end submodule MOM_energetic_PBL_s
