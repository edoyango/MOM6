submodule (MOM_oda_incupd) MOM_oda_incupd_s
#include <MOM_memory.h>
  implicit none
contains
module procedure initialize_oda_incupd_fixed
...
end procedure initialize_oda_incupd_fixed
module procedure initialize_oda_incupd
...
end procedure initialize_oda_incupd
module procedure set_up_oda_incupd_field
  integer :: i, j, k
...
end procedure set_up_oda_incupd_field
module procedure set_up_oda_incupd_vel_field
  integer :: i, j, k
  if (.not.associated(CS)) return
...
end procedure set_up_oda_incupd_vel_field
module procedure calc_oda_increments
...
end procedure calc_oda_increments
module procedure apply_oda_incupd
  real, allocatable, dimension(:) :: tmp_val2  ! data values remapped to increment grid, in rescaled units
  real, dimension(SZK_(GV)) :: tmp_val1        ! data values on the model grid, in rescaled units
  real, dimension(SZK_(GV)) :: hu, hv          ! A column of thicknesses at u or v points [H ~> m or kg m-2]
...
end procedure apply_oda_incupd
module procedure output_oda_incupd_inc
...
end procedure output_oda_incupd_inc
module procedure init_oda_incupd_diags
  if (.not.associated(CS)) return
...
end procedure init_oda_incupd_diags
module procedure oda_incupd_end
  integer :: m
  if (.not.associated(CS)) return
...
end procedure oda_incupd_end
end submodule MOM_oda_incupd_s
