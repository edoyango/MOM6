submodule (MOM_open_boundary) MOM_open_boundary_s
#include <MOM_memory.h>
  implicit none
contains
module procedure open_boundary_config
...
end procedure open_boundary_config
module procedure open_boundary_setup_vert
...
end procedure open_boundary_setup_vert
module procedure segment_determine_required_fields
...
end procedure segment_determine_required_fields
module procedure find_phys_field_index
  integer :: i
...
end procedure find_phys_field_index
module procedure OBC_any_IO
  integer :: m, n
  logical :: use_IO
  use_IO = .false.
  do n=1,OBC%number_of_segments
...
end procedure OBC_any_IO
module procedure allocate_segment_field_data
...
end procedure allocate_segment_field_data
module procedure initialize_segment_data
  ...
  call OBC_any_IO(OBC)
end procedure initialize_segment_data
module procedure field_is_on_face
...
end procedure field_is_on_face
module procedure field_is_tidal
  field_is_tidal = ((index(name, 'phase') > 0) .or. (index(name, 'amp') > 0))
end procedure field_is_tidal
module procedure set_segnum_signs
  integer :: i, j
...
end procedure set_segnum_signs
module procedure scale_factor_from_name
  integer :: m
...
end procedure scale_factor_from_name
module procedure initialize_obc_tides
  ...
  enddo
end procedure initialize_obc_tides
module procedure setup_segment_indices
...
end procedure setup_segment_indices
module procedure setup_u_point_obc
...
end procedure setup_u_point_obc
module procedure setup_v_point_obc
...
end procedure setup_v_point_obc
module procedure parse_segment_str
...
end procedure parse_segment_str
module procedure parse_segment_manifest_str
  ...
  enddo
end procedure parse_segment_manifest_str
module procedure parse_segment_data_str
...
end procedure parse_segment_data_str
module procedure parse_for_tracer_reservoirs
...
end procedure parse_for_tracer_reservoirs
module procedure open_boundary_halo_update
  integer :: m
...
end procedure open_boundary_halo_update
module procedure open_boundary_query
...
end procedure open_boundary_query
module procedure open_boundary_dealloc
  ...
  deallocate(OBC)
end procedure open_boundary_dealloc
module procedure open_boundary_end
  call open_boundary_dealloc(OBC)
end procedure open_boundary_end
module procedure open_boundary_impose_normal_slope
  integer :: i, j, n
  type(OBC_segment_type), pointer :: segment => NULL()
  if (.not.associated(OBC)) return
...
end procedure open_boundary_impose_normal_slope
module procedure open_boundary_impose_land_mask
  integer :: i, j, n
  type(OBC_segment_type), pointer :: segment => NULL()
  logical :: any_U, any_V
  if (.not.associated(OBC)) return
...
end procedure open_boundary_impose_land_mask
module procedure setup_OBC_tracer_reservoirs
  type(OBC_segment_type), pointer :: segment => NULL()
...
end procedure setup_OBC_tracer_reservoirs
module procedure setup_OBC_thickness_reservoirs
  type(OBC_segment_type), pointer :: segment => NULL()
...
end procedure setup_OBC_thickness_reservoirs
module procedure set_initialized_OBC_tracer_reservoirs
  character(len=12) :: x_var_name, y_var_name
  integer :: m
  do m=1,OBC%ntr
...
end procedure set_initialized_OBC_tracer_reservoirs
module procedure copy_thickness_reservoirs
  type(OBC_segment_type), pointer :: segment => NULL()
  integer :: i, j, k, n
  logical :: sym
  if (.not.associated(OBC)) return
...
end procedure copy_thickness_reservoirs
module procedure radiation_open_bdry_conds
...
end procedure radiation_open_bdry_conds
module procedure open_boundary_apply_normal_flow
  integer :: i, j, k, n
  type(OBC_segment_type), pointer :: segment => NULL()
...
end procedure open_boundary_apply_normal_flow
module procedure open_boundary_zero_normal_flow
  integer :: i, j, k, n
  type(OBC_segment_type), pointer :: segment => NULL()
  if (.not.associated(OBC)) return ! Bail out if OBC is not available
...
end procedure open_boundary_zero_normal_flow
module procedure gradient_at_q_points
  integer :: i,j,k
  if (.not. segment%on_pe) return
...
end procedure gradient_at_q_points
module procedure lookup_seg_field
  integer :: n
  lookup_seg_field = -1
...
end procedure lookup_seg_field
module procedure get_tracer_index
  integer :: it
  get_tracer_index = -1
  it = 1
  do while(allocated(OBC_seg%tr_Reg%Tr(it)%t))
    ...
    it = it + 1
  enddo
end procedure get_tracer_index
module procedure allocate_OBC_segment_data
...
end procedure allocate_OBC_segment_data
module procedure deallocate_OBC_segment_data
  if (.not. segment%on_pe) return
...
end procedure deallocate_OBC_segment_data
module procedure open_boundary_test_extern_uv
  integer :: i, j, k, n
  if (.not. associated(OBC)) return
...
end procedure open_boundary_test_extern_uv
module procedure open_boundary_test_extern_h
...
end procedure open_boundary_test_extern_h
module procedure read_OBC_segment_data
  integer :: i, j, k, n, m
...
end procedure read_OBC_segment_data
module procedure update_OBC_segment_data
  type(OBC_segment_type), pointer :: segment => NULL()
...
end procedure update_OBC_segment_data
module procedure initialize_OBC_segment_reservoirs
  type(OBC_segment_type), pointer :: segment => NULL()
  integer :: isd, ied, jsd, jed, IsdB, IedB, JsdB, JedB
  ...
  do n=1,OBC%number_of_segments
    ...
    enddo ! end tracer field loop
  enddo ! end segment loop
end procedure initialize_OBC_segment_reservoirs
module procedure update_OBC_ramp
...
end procedure update_OBC_ramp
module procedure register_OBC
  integer :: nobc
  character(len=256) :: mesg    ! Message for error messages.
...
end procedure register_OBC
module procedure OBC_registry_init
...
end procedure OBC_registry_init
module procedure register_file_OBC
...
end procedure register_file_OBC
module procedure file_OBC_end
  if (associated(CS)) then
    deallocate(CS)
  endif
end procedure file_OBC_end
module procedure segment_tracer_registry_init
  integer, save :: init_calls = 0
# include "version_variable.h"
...
end procedure segment_tracer_registry_init
module procedure segment_thickness_reservoir_init
...
end procedure segment_thickness_reservoir_init
module procedure register_segment_tracer
  real :: rescale ! A multiplicatively corrected scaling factor, in units like [S ppt-1 ~> 1] for
...
end procedure register_segment_tracer
module procedure segment_tracer_registry_end
  integer n
...
end procedure segment_tracer_registry_end
module procedure segment_thickness_registry_end
  if (associated(Reg)) then
    if (allocated(Reg%h)) deallocate(Reg%h)
    if (allocated(Reg%h_res)) deallocate(Reg%h_res)
    deallocate(Reg)
  endif
end procedure segment_thickness_registry_end
module procedure register_temp_salt_segments
  integer :: n, ntr_id
...
end procedure register_temp_salt_segments
module procedure set_obgc_segments_props
...
end procedure set_obgc_segments_props
module procedure get_obgc_segments_props
  ...
  node => node%next
end procedure get_obgc_segments_props
module procedure register_obgc_segments
  integer :: ntr_id
  integer :: n, m
  type(OBC_segment_type), pointer :: segment => NULL() ! pointer to segment type list
  type(tracer_type),      pointer :: tr_ptr => NULL()
  ...
  enddo
end procedure register_obgc_segments
module procedure fill_obgc_segments
...
end procedure fill_obgc_segments
module procedure fill_temp_salt_segments
...
end procedure fill_temp_salt_segments
module procedure fill_thickness_segments
  integer :: isd, ied, IsdB, IedB, jsd, jed, JsdB, JedB, n, nz
  integer :: i, j, k
  type(OBC_segment_type), pointer :: segment => NULL() ! pointer to segment type list
  if (.not. associated(OBC)) return
...
end procedure fill_thickness_segments
module procedure mask_outside_OBCs
  integer :: i, j
  ...
  deallocate(color)
  deallocate(color2)
end procedure mask_outside_OBCs
module procedure flood_fill
  integer :: i, j, ncount
  ncount = 1
  do while (ncount > 0)
    ncount = 0
...
end procedure flood_fill2
module procedure open_boundary_register_restarts
  type(vardesc) :: vd(2)
  integer       :: m
...
end procedure open_boundary_register_restarts
module procedure update_segment_tracer_reservoirs
  type(OBC_segment_type), pointer :: segment => NULL()
  ...
  enddo
end procedure update_segment_tracer_reservoirs
module procedure update_segment_thickness_reservoirs
  type(OBC_segment_type), pointer :: segment=>NULL()
...
end procedure update_segment_thickness_reservoirs
module procedure remap_OBC_fields
...
end procedure remap_OBC_fields
module procedure adjustSegmentEtaToFitBathymetry
...
end procedure adjustSegmentEtaToFitBathymetry
module procedure rotate_OBC_config
  integer :: c, n, l_seg
...
end procedure rotate_OBC_config
module procedure rotate_OBC_segment_config
...
end procedure rotate_OBC_segment_config
module procedure rotate_OBC_segment_direction
  integer :: qturns ! The number of quarter turns in the range of 0 to 3
  qturns = modulo(turns, 4)
...
end procedure rotate_OBC_segment_direction
module procedure rotated_field_name
  if (modulo(turns, 2) /= 0) then
...
end procedure rotated_field_name
module procedure allocate_rotated_seg_data
...
end procedure allocate_rotated_seg_data
module procedure write_OBC_info
  type(OBC_segment_type), pointer :: segment => NULL() ! pointer to segment type list
  integer :: turns      ! Number of index quarter turns
...
end procedure write_OBC_info
module procedure chksum_OBC_segments
  integer :: n          ! The segment number reported in output
  integer :: n_seg      ! The internal segment number
  do n=1,OBC%number_of_segments
    n_seg = n ; if (OBC%reverse_segment_order) n_seg = OBC%number_of_segments + 1 - n
...
end procedure chksum_OBC_segments
module procedure chksum_OBC_segment_data
...
end procedure chksum_OBC_segment_data
end submodule MOM_open_boundary_s
