...
end procedure set_dtbt
module procedure apply_u_velocity_OBCs
...
end procedure apply_u_velocity_OBCs
module procedure apply_v_velocity_OBCs
  real :: vel_prev    ! The previous velocity [L T-1 ~> m s-1].
  real :: cfl         ! The CFL number at the point in question [nondim]
...
end procedure apply_v_velocity_OBCs
module procedure initialize_BT_OBC
  real, dimension(SZIBW_(CS),SZJW_(CS)) :: &
...
end procedure initialize_BT_OBC
module procedure set_up_BT_OBC
...
end procedure set_up_BT_OBC
module procedure destroy_BT_OBC
...
end procedure btcalc
module procedure find_uhbt
  if (u == 0.0) then
    uhbt = 0.0
...
end procedure find_uhbt
module procedure find_duhbt_du
  if (u == 0.0) then
...
end procedure find_duhbt_du
module procedure uhbt_to_ubt
...
end procedure uhbt_to_ubt
module procedure find_vhbt
  if (v == 0.0) then
    vhbt = 0.0
...
end procedure find_vhbt
module procedure find_dvhbt_dv
  if (v == 0.0) then
...
end procedure find_dvhbt_dv
module procedure vhbt_to_vbt
...
end procedure vhbt_to_vbt
module procedure set_local_BT_cont_types
...
end procedure adjust_local_BT_cont_types
module procedure BT_cont_to_face_areas
  integer :: i, j, is, ie, js, je, hs
  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec
...
module procedure find_face_areas
...
end procedure barotropic_init
module procedure barotropic_get_tav
  integer :: i,j
  do j=G%jsc,G%jec ; do I=G%isc-1,G%iec
...
end procedure barotropic_get_tav
module procedure barotropic_end
...
end procedure barotropic_end
module procedure register_barotropic_restarts
  type(vardesc) :: vd(3)
  character(len=40)  :: mdl = "MOM_barotropic"  ! This module's name.
...
end procedure register_barotropic_restarts
end submodule MOM_barotropic_s
