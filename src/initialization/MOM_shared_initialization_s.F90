submodule (MOM_shared_initialization) MOM_shared_initialization_s
  implicit none
contains
module procedure MOM_shared_init_init
...
end procedure MOM_shared_init_init
module procedure MOM_initialize_rotation
...
end procedure MOM_initialize_rotation
module procedure MOM_calculate_grad_Coriolis
...
end procedure MOM_calculate_grad_Coriolis
module procedure diagnoseMaximumDepth
  integer :: i,j
...
end procedure diagnoseMaximumDepth
module procedure set_meanSL_from_file
  ...
  call callTree_leave(trim(mdl)//'()')
end procedure set_meanSL_from_file
module procedure initialize_topography_from_file
  ...
  call callTree_leave(trim(mdl)//'()')
end procedure initialize_topography_from_file
module procedure apply_topography_edits_from_file
  ...
  call callTree_leave(trim(mdl)//'()')
end procedure apply_topography_edits_from_file
module procedure initialize_topography_named
  ...
  call callTree_leave(trim(mdl)//'()')
end procedure initialize_topography_named
module procedure limit_topography
  integer :: i, j
  ...
  call callTree_leave(trim(mdl)//'()')
end procedure limit_topography
module procedure set_rotation_planetary
  ...
  call callTree_leave(trim(mdl)//'()')
end procedure set_rotation_planetary
module procedure set_rotation_beta_plane
  integer :: I, J
  ...
  call callTree_leave(trim(mdl)//'()')
end procedure set_rotation_beta_plane
module procedure initialize_grid_rotation_angle
...
end procedure initialize_grid_rotation_angle
module procedure modulo_around_point
  if (Lx > 0.0) then
    x_mod = modulo(x - (xc - 0.5*Lx), Lx) + (xc - 0.5*Lx)
  else
    x_mod = x
  endif
end procedure modulo_around_point
module procedure reset_face_lengths_named
...
end procedure reset_face_lengths_named
module procedure reset_face_lengths_file
  ...
  call callTree_leave(trim(mdl)//'()')
end procedure reset_face_lengths_file
module procedure reset_face_lengths_list
  ...
  call callTree_leave(trim(mdl)//'()')
end procedure reset_face_lengths_list
module procedure read_face_length_list
...
end procedure read_face_length_list
module procedure set_subgrid_topo_at_vel_from_file
  character(len=200) :: filename, topo_file, inputdir ! Strings for file/path
  ...
  call callTree_leave(trim(mdl)//'()')
end procedure set_subgrid_topo_at_vel_from_file
...
module procedure compute_global_grid_integrals
...
end procedure compute_global_grid_integrals
module procedure write_ocean_geometry_file
...
end procedure write_ocean_geometry_file
end submodule MOM_shared_initialization_s
