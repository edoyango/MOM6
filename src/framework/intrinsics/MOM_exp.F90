...
!! Hoisted to module scope (E4): a function-local parameter array is
!! implemented as a static variable, which nvfortran's -Minline refuses to
!! cross-file inline ("subprogram not inlined -- static variable during
!! crossing files"), independent of submodule structure -- confirmed with
!! an isolated test while investigating why -Mextract/-Minline could not
!! fully eliminate exp_repro's call overhead.
...
!! Coefficients (exp_remez_c) are module-scope, not function-local -- see the
!! comment on exp_remez_c's declaration above.
