submodule (MOM_random) MOM_random_s
  implicit none
contains
module procedure random_01
  random_01 = getRandomReal(CS%stream0d)

end procedure random_01
module procedure random_01_CB
...
end procedure random_01_CB
module procedure random_norm
  integer :: i
...
end procedure random_norm
module procedure random_2d_01
  integer :: i,j
  do j = HI%jsd,HI%jed
    do i = HI%isd,HI%ied
...
end procedure random_2d_01
module procedure random_2d_norm
  integer :: i,j,n
  do j = HI%jsd,HI%jed
    do i = HI%isd,HI%ied
...
end procedure random_2d_norm
module procedure random_0d_constructor
  integer :: tseed
  tseed = seed_from_time(Time)
  tseed = ieor(tseed, seed)
  CS%stream0d = new_RandomNumberSequence(tseed)

end procedure random_0d_constructor
module procedure random_2d_constructor
...
end procedure random_2d_constructor
module procedure seed_from_time
...
end procedure seed_from_time
module procedure seed_from_index
  integer :: ig, jg, ni, nj
  ni = HI%niglobal
  nj = HI%njglobal
...
end procedure seed_from_index
module procedure random_destruct
  if (allocated(CS%stream2d)) deallocate(CS%stream2d)
  !deallocate(CS)
end procedure random_destruct
module procedure new_RandomNumberSequence
  integer :: i
...
end procedure new_RandomNumberSequence
module procedure getRandomInt
...
end procedure getRandomInt
module procedure getRandomReal
  integer :: localInt
...
end procedure nextState
module procedure temper
  integer :: x
  x      = ieor(y, ishft(y, -11))
  x      = ieor(x, iand(ishft(x,  7), TMASKB))
  x      = ieor(x, iand(ishft(x, 15), TMASKC))
  temper = ieor(x, ishft(x, -18))
end procedure temper
module procedure random_unit_tests
...
end procedure random_unit_tests
module procedure test_fn
...
end procedure test_fn
end submodule MOM_random_s
