...
type, public :: MOM_generic_tracer_CS ; private
  ...
  interface
module function register_MOM_generic_tracer(HI, GV, param_file, CS, tr_Reg, restart_CS)
...
module subroutine register_MOM_generic_tracer_segments(CS, GV, OBC, tr_Reg, param_file)
  type(MOM_generic_tracer_CS), pointer    :: CS         !< Pointer to the control structure for this module.
  type(verticalGrid_type),     intent(in) :: GV         !< The ocean's vertical grid structure
...
module subroutine initialize_MOM_generic_tracer(restart, day, G, GV, US, h, tv, param_file, diag, OBC, &
...
module subroutine MOM_generic_tracer_column_physics(h_old, h_new, ea, eb, fluxes, Hml, dt, G, GV, US, CS, tv, optics, &
...
module function MOM_generic_tracer_stock(h, stocks, G, GV, CS, names, units, stock_index)
  type(ocean_grid_type),              intent(in)    :: G    !< The ocean's grid structure
  type(verticalGrid_type),            intent(in)    :: GV   !< The ocean's vertical grid structure
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)), intent(in) :: h !< Layer thicknesses [H ~> m or kg m-2]
...
module function MOM_generic_tracer_min_max(ind_start, got_minmax, gmin, gmax, G, CS, names, units, &
...
module subroutine MOM_generic_tracer_surface_state(sfc_state, h, G, GV, CS)
  type(ocean_grid_type),                 intent(in)    :: G    !< The ocean's grid structure
  type(verticalGrid_type),               intent(in)    :: GV   !< The ocean's vertical grid structure
...
module subroutine MOM_generic_flux_init(verbosity)
  integer, optional, intent(in) :: verbosity  !< A 0-9 integer indicating a level of verbosity.

end subroutine MOM_generic_flux_init
module subroutine MOM_generic_tracer_fluxes_accumulate(flux_tmp, weight)
...
module subroutine MOM_generic_tracer_get(name,member,array, CS)
...
module subroutine end_MOM_generic_tracer(CS)
  type(MOM_generic_tracer_CS), pointer :: CS   !< Pointer to the control structure for this module.
