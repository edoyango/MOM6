...
module function MOM_namelist_file(filepath) result(iounit)
...
module subroutine open_file(IO_handle, filename, action, MOM_domain, threading, fileset)
...
module function find_index(vec) result(loc)
...
module subroutine read_vector_2d(filename, u_fieldname, v_fieldname, u_data, v_data, MOM_Domain, &
...
module subroutine read_vector_3d(filename, u_fieldname, v_fieldname, u_data, v_data, MOM_Domain, &
                          timelevel, stagger, scalar_pair, scale)
  character(len=*),       intent(in)    :: filename  !< The name of the file to read
  character(len=*),       intent(in)    :: u_fieldname !< The variable name of the u data in the file
  character(len=*),       intent(in)    :: v_fieldname !< The variable name of the v data in the file
...
module subroutine find_varname_in_file(fileobj, fieldname, err_header, filename, var_to_read, has_time_dim, timelevel)
...
module subroutine prepare_to_read_var(fileobj, fieldname, err_header, filename, var_to_read, &
...
module subroutine MOM_register_variable_axes(fileObj, variableName, filename, position)
...
module subroutine categorize_axes(fileObj, filename, ndims, dim_names, is_x, is_y, is_t)
...
module subroutine categorize_axis_from_units(unit_string, is_x, is_y)
...
module subroutine categorize_axis_from_name(dimname, is_x, is_y)
...
integer module function write_time_if_later(IO_handle, field_time)
  type(file_type), intent(inout) :: IO_handle  !< Handle for a file that is open for writing
...
module subroutine write_metadata_axis(IO_handle, axis, name, units, longname, cartesian, sense, domain, data, &
                               edge_axis, calendar)
  type(file_type),            intent(in)    :: IO_handle  !< Handle for a file that is open for writing
...
module function find_unlimited_dimension_name(fileobj) result(label)
